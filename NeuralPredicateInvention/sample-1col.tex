...
\usepackage{euler}  
\usepackage{xcolor}
\usetikzlibrary{positioning,automata,arrows, matrix}
\tikzset{
...
\begin{document}
	...
	%% Tell some reasons if possible
		...
		\begin{figure*}[t]
		\centering
		\includegraphics{scripts/intro.tikz}
	...
	\begin{table}[t]
		...
		\begin{tabular}{c|c|c}
			...
			& NeSy-ILP &  NeSy-$ \pi $ \\
			\hline 
			\multirow{2}*{Const} & \multicolumn{2}{c}{$\mathtt{ Object:O1/O2, Color: Blue/Pink/Green}$}\\
			& \multicolumn{2}{c}{$\mathtt{Shape:Sphere/Cube}$ ,  $\mathtt{Direction: Left/Right/Front/Behind}$}\\
			\hline 
			Basic-Pred &\multicolumn{2}{c}{$\mathtt{color/2/obj,color;shape/2/obj,shape; dir/2/obj,obj}$} \\ 
			\hline
			\multirow{3}*{BK} & $\mathtt{b\_sp(O1):-color(O1,Blue),shape(O1,Sphere).}$ & \multirow{3}*{-} \\
	...
	\end{figure*}
		...
		\begin{figure*}[t]
	\centering
	\includegraphics[width=0.8\textwidth]{scripts/polar.tikz}
	...
	\begin{figure*}[t]
		\centering
		\includegraphics[width=0.8\textwidth]{scripts/fsm.tikz}
	...
	\begin{figure}[!htb]
		...
		\subfigure[Cross]{\includegraphics[width=0.24\textwidth]{img/3DKP/cross_same_shape.png}} 
...
rho(O3,O4,rho2).
	...
	\begin{table}
		...
		\begin{tabular}{c|c|c|c|c|c|c|c|c|c|c}
			\hline
			\toprule
			\multirow{2}*{Dataset} & \multirow{2}*{Patterns} &\multirow{2}*{BS} &  \multicolumn{2}{c|}{Iterations} &\multicolumn{2}{c|}{Time} & \multicolumn{2}{c|}{Accuracy} & \multicolumn{2}{c}{\# of Ne-Preds} \\
			\cline{4-11}
